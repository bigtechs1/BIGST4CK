...
module.exports = {
    name: "welcome", aliases: ["wel"], category: "group",
    code: async (ctx) => {
        ...
        if (sub === 'on') { configData[chatId].enabled = true; saveConfig(configData); await sock.sendMessage(chatId, { text: `» Welcome messages enabled.` }, { quoted: msg }); return; }
        if (sub === 'off') { configData[chatId].enabled = false; saveConfig(configData); await sock.sendMessage(chatId, { text: `» Welcome messages disabled.` }, { quoted: msg }); return; }
        if (sub === 'setimage') {
            if (args.length > 1) { const url = args.slice(1).join(' ').trim(); if (url.startsWith('http')) { configData[chatId].imageUrl = url; saveConfig(configData); await sock.sendMessage(chatId, { text: `» Welcome image set.` }, { quoted: msg }); return; } else { await sock.sendMessage(chatId, { text: `» Invalid URL.` }, { quoted: msg }); return; } }
            else { await sock.sendMessage(chatId, { text: `» Usage: ${prefix}welcome setimage <image_url>` }, { quoted: msg }); return; }
        }
        ...
        await sock.sendMessage(chatId, { text: `» Welcome Settings\n› Status: ${status}\n› Image: ${image}\n› Commands: ${prefix}welcome on/off | ${prefix}welcome setimage <url> | ${prefix}welcome description` }, { quoted: msg });
    }
};
module.exports.handleWelcomeEvent = async function handleWelcomeEvent(sock, groupId, participant, pushName) {
    const configData = loadConfig(); if (!configData[groupId] || !configData[groupId].enabled) return;
    const imageUrl = configData[groupId].imageUrl || null; const desc = await getGroupDescription(sock, groupId); const userName = pushName || participant.split('@')[0];
    let body = `» Welcome @${userName} to the group! 🎉\n\n» Group Description:\n${desc}\n\n» Respect the admins and enjoy your stay! 🙏\n${FOOTER}`;
