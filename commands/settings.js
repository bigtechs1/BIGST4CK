...
// ─── Helper: load config from JSON files ────────────────
...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        const features = {
            ...
            antidelete: 'Detect deleted messages',
            antilink: 'Block links in groups',
            antimention: 'Block mentions in groups',
            autostatus: 'Auto-view + like status',
            chatbot: 'AI chatbot'
        };
        ...
        if (args.length >= 2) {
            ...
            const success = toggleFeature(feature, enable);
            if (success) {
                await sock.sendMessage(chatId, {
                    text: `» ${feature} turned ${action.toUpperCase()}.\n${FOOTER}`
                }, { quoted: msg });
            } else {
                await sock.sendMessage(chatId, {
        ...
        const ownerNumber = ctx.sender.jid.split('@')[0];
        const greeting = 'Owner';

        // ─── BODY TEXT (greeting + date) ──────────────
        const bodyText =
            `${greeting}, @${ownerNumber}\n` +
            `${dateEN} · ${timeEN}`;

        // ─── FOOTER TEXT (all settings) ────────────────
        const statuses = {};
        for (const [key] of Object.entries(features)) {
            statuses[key] = loadFeatureStatus(key);
        }
        ...
        const footerText =
            ...
            `  › Mode      : ${config.system?.mode || 'public'}\n` +
            `  › Uptime    : ${getUptime()}\n` +
            `\n` +
            `*»* *FEATURES*\n` +
        ...
        await new AIRich(sock)
            ...
            .setSubtitle(`Settings · ${timeEN}`)
            .setBody(bodyText)
            .setFooter(footerText)
            .setThumbnail(ppUrl)
            .addTip('Use .setprefix <new> to change prefix')
