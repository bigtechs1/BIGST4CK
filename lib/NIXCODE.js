// lib/NIXCODE.js – CommonJS version
...
class AIRich extends BaseBuilder {
    ...
    addText(text, { hyperlink = true, citation = true, latex = true } = {}) {
        ...
        const inline_entities = extractedIE.ie.map(({ type, ie }) => {
            ...
            return {
                ...
                metadata: {
                    latex_expression: ie.text,
                    latex_image: { url: ie.url, width, height },
                    font_height: Number(ie.font_height) || 83.333333333333,
                    padding: Number(ie.padding) || 15,
                    __typename: 'GenAILatexItem'
                }
    ...
    addReels(reelsItems = []) {
        ...
        this._sections.push(AIRich.newLayout('HScroll', reelsItems.map((item) => ({
    ...
    addImage(imageUrl) {
        ...
        const imageUrls = Array.isArray(imageUrl)
            ? imageUrl.map((url) => ({ imagePreviewUrl: url, imageHighResUrl: url, sourceUrl: String.fromCharCode(104, 116, 116, 112, 115, 58, 47, 47, 102, 105, 111, 114, 97, 46, 110, 105, 120, 101, 108, 46, 109, 121, 46, 105, 100, 47) }))
            : [{ imagePreviewUrl: imageUrl, imageHighResUrl: imageUrl, sourceUrl: String.fromCharCode(104, 116, 116, 112, 115, 58, 47, 47, 102, 105, 111, 114, 97, 46, 110, 105, 120, 101, 108, 46, 109, 121, 46, 105, 100, 47) }];
