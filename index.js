// BIGST4CK - A BIN WHATSAPP BOT{CLEAN VERSION}
...
const pinoLogger = pino({
    ...
    transport: {
        target: 'pino-pretty',
        options: { colorize: true, translateTime: 'SYS:standard', ignore: 'pid,hostname', singleLine: false, messageFormat: '{levelLabel} - {msg}' }
    }
});

const originalLog = console.log;
console.log = function(...args) { originalLog(...args); };

// ─── Globals ────────────────────────────────────────────
global.botname = config.botName;
global.themeemoji = '›';
global.channelLink = config.channellink || "https://whatsapp.com/channel/0029VbDJJY19WtC1T0Vgqp0v";
...
const question = (text) => {
    ...
    return Promise.resolve(config.ownerNumber || "255636756591");
};

async function chooseStartupMode() {
    const settingMode = config.mode?.toLowerCase() === 'telegram' ? 'telegram' : 'whatsapp';
    if (!rl) {
        console.log(chalk.yellow(`⚠️ Terminal input unavailable, using mode: ${settingMode}`));
        return settingMode;
    }
    console.log(chalk.bgBlue.white("\n  🚀  BIGST4CK STARTUP MODE  🚀  \n"));
    console.log('1) WhatsApp');
    console.log('2) Telegram');
    console.log(`3) Use mode from config (${settingMode})`);
...
async function startBIGST4CK() {
    try {
        console.log('\n' + chalk.bgBlue.white(`  🚀  STARTING ${config.botName}  🚀  `) + '\n');
        ...
        sock.ev.on("messages.upsert", async chatUpdate => {
            try {
                ...
                if (!err.message?.includes("No session found") && !err.message?.includes("No matching sessions") && !err.message?.includes("timed out waiting")) {
        ...
        sock.ev.on("call", async (callData) => {
            try { await handleAnticall(sock, { call: callData }); } catch (err) {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                console.log(chalk.green.bold(`✅ ${config.botName} Online!\n`));
                // Welcome message to owner
                try {
                    const ownerJid = config.ownerNumber.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
                    const thumbnailUrl = config.thumbnail || "https://files.catbox.moe/0hmdof.png";
                    const ramUsage = (process.memoryUsage().rss / 1024 / 1024).toFixed(2);
                    const body = `› *Welcome to ${config.botName}!* ›\n› Status: Online & Active\n› RAM: ${ramUsage} MB\n› ${config.system.prefix}menu for usage\n${config.footer}`;
                    const welcomeBtn = new Button(sock)
                        .setTitle(`🚀 ${config.botName} BOT`)
                        .setBody(body)
                        .setFooter(config.footer || `© ${config.botName}`)
                        .setImage(thumbnailUrl)
                        .addUrl('📢 Channel', config.channellink || global.channelLink)
                        .addUrl('💬 Group', config.groupLink || 'https://chat.whatsapp.com/EWlNm6bMYJCELwzvnmboyC')
                        .addReply('📋 Menu', 'menu')
                        .addReply('👤 Owner', 'owner');
                    await welcomeBtn.send(ownerJid);
                } catch (e) { console.log(chalk.yellow(`⚠️ Could not send rich welcome: ${e.message}`)); }
        ...
        if (pairingCode && !sock.authState.creds.registered) {
            ...
            setTimeout(async () => {
                try {
                    let code = await sock.requestPairingCode(num, config.system?.customPairingCode || "BIGTECHS");
                    console.log(chalk.bgCyan.black("  🔐  YOUR CUSTOM PAIRING CODE  🔐  "));
                    console.log(chalk.white.bold("  CODE: ") + chalk.green.bold(config.system?.customPairingCode || "BIGTECHS"));
...
async function initializeBot() {
    ...
    if (startupMode === 'telegram') {
        ...
        try {
            ...
            console.error(chalk.yellow("Please install: npm install node-telegram-bot-api"));
            process.exit(1);
        }
    } else {
